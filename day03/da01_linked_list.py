...
def insertNode(findData, insertData):
    ...
    if head.getData() == findData:
        ...
        head = node # head는 새 node 로 설정
        return # 더이상 밑으로 싱행안되도록 함수
...
def findNode(findData):
    ...
    return None # 찾는 데이터가 없다    

# 연결 리스트 생성 구현

if __name__ == '__main__':   # 내가 시작 모듈일 때
    ...
    for name in dataArray[1:]: # 정연 부터 사용 
        ...
        prev.setLink(node) # 이번노드에 현재노드를 연결
        memory.append(node)
