...
def findVertex(g, findVtx):
    ...
    while len(stack) != 0 :
        ...
        for vertex in range(SIZE):
            if g.graph[current][vertex] != 0: # 도착점이 이미 방문했으면
...
while len(newAry) > (SIZE - 1): # 간선의 수가 (정점수 -1) 될 때까지 반복
    ...
    endYN = findVertex(G, end) # 광주에연결된 간선 확인

    if startYN and endYN: # 둘 다 다른 간선있으니까 지금 간선 지워도 됨
        del(newAry[index])
    else:
