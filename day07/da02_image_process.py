...
def sortQuickN(ary, start, end):
    ...
    mid = low # 모든 정렬을 한바퀴돌면 low는 리스트 중간쯤 가있는다
...
# 생상들을 정리. midValue보다 작으면 검은색, 중간값보다 크면 흰색
...
for i in range(h):
    for k in range(w):
        r = g = b = photoAry[pos]
        pos += 1
        photo.put(f'#{r:02x}{g:02x}{b:02x}',(i,k)) # photo에 각 위치의 이미지 생각을 photoAry에 들어있는 값으로 변경
